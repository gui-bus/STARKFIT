...
import { UsersIcon, BuildingsIcon, LightningIcon, TargetIcon, GlobeIcon, StarIcon, ArrowRightIcon } from "@phosphor-icons/react";
import Image from "next/image";

const stats = [
  { label: "Membros Elite", value: "5k+", icon: <UsersIcon weight="bold" className="text-primary w-6 h-6" /> },
  { label: "Unidades", value: "12", icon: <BuildingsIcon weight="bold" className="text-primary w-6 h-6" /> },
  { label: "Performance", value: "100%", icon: <LightningIcon weight="bold" className="text-primary w-6 h-6" /> },
  { label: "Resultados", value: "Focado", icon: <TargetIcon weight="bold" className="text-primary w-6 h-6" /> },
];

const AboutSection = () => {
  return (
    <section id="About" className="py-48 bg-black overflow-hidden relative">
              ...
              {stats.map((stat, i) => (
                <div key={i} className="bg-white/5 p-10 rounded-[3rem] border border-white/5 hover:border-primary/30 transition-all duration-500 group shadow-2xl">
                  <div className="flex items-center gap-5 mb-5">
                    <div className="p-3.5 bg-black rounded-2xl border border-white/10 group-hover:bg-primary transition-all duration-500 shadow-xl group-hover:scale-110">
