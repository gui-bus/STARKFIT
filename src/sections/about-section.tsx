...
import { UsersIcon, BuildingsIcon, LightningIcon, TargetIcon, GlobeIcon, ArrowRightIcon } from "@phosphor-icons/react";
...
const AboutSection = () => {
  ...
  const stats = [
    { label: t("stats.members"), value: "5k+", icon: <UsersIcon weight="bold" className="w-6 h-6 text-primary" /> },
    { label: t("stats.units"), value: "12", icon: <GlobeIcon weight="bold" className="w-6 h-6 text-primary" /> },
    { label: t("stats.performance"), value: "100%", icon: <LightningIcon weight="bold" className="w-6 h-6 text-primary" /> },
    { label: t("stats.results"), value: t("stats.resultsValue"), icon: <TargetIcon weight="bold" className="w-6 h-6 text-primary" /> },
  ];
            ...
            <div className="relative z-10 w-full aspect-[4/5] bg-white/5 rounded-[4.5rem] overflow-hidden border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.8)]">
               <Image 
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                   <p className="text-8xl font-black text-white/5 uppercase select-none leading-none mb-6 tracking-tighter">{t("discipline")}</p>
                   <div className="px-8 py-3 rounded-full border border-primary/30 backdrop-blur-md bg-primary/5 shadow-2xl">
                      <p className="text-sm font-black text-primary italic uppercase tracking-[0.5em]">{t("legacy")}</p>
                   </div>
                </div>
            </div>
          ...
          >
            <div className="space-y-8">
              ...
              <h2 className="text-7xl md:text-[9.5rem] font-black uppercase text-white leading-[0.8] tracking-tighter">
                {t("title1")} <br /> <span className="text-primary italic">{t("title2")}</span>
              </h2>
            ...
            <div className="pt-10">
               <button className="flex items-center gap-6 group outline-none">
                  ...
                  <div className="space-y-1">
                     <p className="text-white font-black italic uppercase tracking-widest leading-none text-lg">{t("cta")}</p>
